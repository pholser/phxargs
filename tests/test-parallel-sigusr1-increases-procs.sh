...
# Send SIGUSR1 to phxargs while it is running with -P 1.
# phxargs should handle the signal gracefully and still produce all output.
...
/usr/bin/env -i "$phx_build_dir/phxargs" -P 1 -n 1 echo \
  < "$phx_test_input" \
  > "$phx_actual_output" \
  2> "$phx_actual_error" &
phxargs_pid=$!
